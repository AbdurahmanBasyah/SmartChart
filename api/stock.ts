...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const rawSymbol = decodeURIComponent(Array.isArray(symbolQuery) ? symbolQuery[0] : String(symbolQuery));

    let cleanTicker = rawSymbol.trim().toUpperCase().replace('.JK', '');
    if (cleanTicker === 'IHSG' || cleanTicker === 'JKSE' || cleanTicker === '^JKSE' || cleanTicker === '%5EJKSE') {
      cleanTicker = '^JKSE';
    }
    ...
    const displayName = cleanTicker === '^JKSE' ? 'Indeks Harga Saham Gabungan (IHSG)' : `${cleanTicker} Indonesia Tbk.`;
    const fallbackCandles = generateCandles(cleanTicker === '^JKSE' ? 7350 : 1500, 0.03, 0.001, 90);
