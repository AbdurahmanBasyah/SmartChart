...
export const CONGLOMERATE_SECTOR_GROUPS: {
    id: string;
    label: string;
    tickers?: string[];
}[] = [
    ...
    {
        id: "PRAJOGO",
        label: "👑 2. Prajogo Pangestu",
        tickers: ["CDIA", "CUAN", "BREN", "PTRO", "TPIA", "SINI", "BRPT"],
    },
    {
        id: "BAKRIE",
        label: "👑 3. Bakrie",
    ...
    },
    {
        id: "BOY_THOHIR",
        label: "👑 4. Boy Thohir",
        tickers: ["MBMA", "ESSA", "MDKA", "AADI", "ADMR", "ADRO", "EMAS"],
    },
    {
        id: "AGUAN",
        label: "👑 5. Aguan",
        tickers: ["CBDK", "ECII", "ERAA", "ERAL", "INPC", "JIHD", "PANI"],
    },
    {
        id: "HAPPY_HAPSORO",
        label: "👑 6. Happy Hapsoro",
        tickers: [
            "ARCI",
            "BUVA",
            "CBRE",
    ...
    },
    {
        id: "COAL",
        label: "⚡ 9. COAL",
        tickers: ["BUMI", "HRUM", "ITMG", "PTBA", "BYAN"],
    },
    {
        id: "HAJI_ISSAM",
        label: "👑 10. HAJI ISSAM",
        tickers: ["FAST", "JARR", "PGUN", "TEBE"],
    },
    {
        id: "HASYIM",
        label: "👑 11. HASYIM",
        tickers: ["DOOH", "INET", "KETR", "WIFI"],
    },
    {
        id: "SALIM",
        label: "👑 12. Salim",
    ...
    },
    {
        id: "INTERNET",
        label: "🌐 13. Internet",
        tickers: ["MORA", "DOOH", "IRSX", "INET", "PADA", "WIFI"],
    },
    {
        id: "DATA_CENTER",
        label: "🌐 13. Data Center",
        tickers: [
            "TLKM",
            "ISAT",
            "DSSA",
    ...
    {
        id: "LOGISTIK",
        label: "🚢 14. Logistik dan perkapalan",
        tickers: ["SOCI", "BULL", "GTSI", "HUMI", "LEAD"],
    },
];
...
export const StockScreener: React.FC<StockScreenerProps> = ({
    ...
    const filteredStocks = stocks.filter((stock) => {
        ...
        if (
            filters.conglomerateFilter &&
            filters.conglomerateFilter !== "ALL"
        ) {
            if (filters.conglomerateFilter === "WATCHLIST") {
                const isWl = watchlist.some((w) => {
                    const cleanW = w.trim().toUpperCase().replace(".JK", "");
                    const stockTick = stock.ticker
                        .replace(".JK", "")
                        .toUpperCase();
                    return (
                        stockTick === cleanW ||
                        (cleanW === "IHSG" && stockTick === "^JKSE")
                    );
                });
                if (!isWl) return false;
            } else {
                const selectedGroup = CONGLOMERATE_SECTOR_GROUPS.find(
                    (g) => g.id === filters.conglomerateFilter,
                );
                if (
                    selectedGroup?.tickers &&
                    selectedGroup.tickers.length > 0
                ) {
        ...
        if (
            ...
            rec?.structure !== filters.structure
        ) {
            return false;
        }
        ...
        if (
            ...
            rec?.primaryZoneType !== filters.zoneType
        ) {
            return false;
        }
    ...
    const sortedStocks = [...filteredStocks].sort((a, b) => {
        ...
        if (sortField === "signal") {
            result =
                getSmcSignalPriorityScore(a) - getSmcSignalPriorityScore(b);
            // Tie-breaker: sort by Risk-Reward ratio descending
            if (result === 0) {
                result =
                    (b.recommendation?.riskRewardRatio ?? 0) -
                    (a.recommendation?.riskRewardRatio ?? 0);
            }
    ...
    const totalPages =
        pageSize === "ALL"
            ? 1
            : Math.max(1, Math.ceil(totalItems / (numPageSize || 1)));
    ...
    const paginatedStocks =
        pageSize === "ALL"
            ? sortedStocks
            : sortedStocks.slice(
                  (safePage - 1) * numPageSize,
                  safePage * numPageSize,
              );

    const startItemIndex =
        totalItems === 0 ? 0 : (safePage - 1) * numPageSize + 1;
    const endItemIndex =
        pageSize === "ALL"
            ? totalItems
            : Math.min(safePage * numPageSize, totalItems);
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-2xl">
                <div className="flex flex-wrap items-center justify-between gap-4 mb-5 border-b border-slate-800 pb-4">
                    <div>
                        <h2 className="text-xl font-bold text-white flex items-center gap-2">
                            ...
                            <span>
                                Smart Money Concepts Stock Screener (IDX)
                            </span>
                        </h2>
                        <p className="text-xs text-slate-400 mt-1">
                            Filter stocks precisely based on FVG, Order Block
                            setups, and Volume Confirmation
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            onClick={() =>
                                setExcludeDowntrend(!excludeDowntrend)
                            }
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-3 text-xs">
                    ...
                    <div>
                        <select
                            ...
                            onChange={(e) =>
                                setFilters({
                                    ...filters,
                                    conglomerateFilter: e.target.value,
                                })
                            }
                    ...
                    <div>
                        ...
                        >
                            <option value="ALL">
                                All SMC Signals (Priority Sorted)
                            </option>
                            <option value="TOP_PICKS">
                                🔥 TOP PICKS (Near Entry & High Upside)
                            </option>
                            <option value="TAPPED_POI_REBOUND">
                                🎯 1. Tapped FVG/OB Recently (Rebound)
                            </option>
                            <option value="ON_BUY_AREA">
                                🎯 2. In Buy Area (On Buy Area)
                            </option>
                            <option value="NEAR_ENTRY">
                                📍 3. Near Entry Area (0-3%)
                            </option>
                            <option value="WAIT_FVG_CREATION">
                                ⏳ 4. Awaiting FVG Creation
                            </option>
                            <option value="WAIT_PULLBACK_FVG">
                                📉 5. Wait Pullback FVG
                            </option>
                            <option value="STRONG_BUY_POI">
                                🔥 6. Discount POI Zone
                            </option>
                            <option value="SIDEWAYS_ACCUMULATION">
                                📦 7. Sideways Accumulation
                            </option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option value="RALLYING">
                                Rallying (Pullback FVG)
                            </option>
                            <option value="SIDEWAYS">
                                Sideways (OB Accumulation)
                            </option>
                        </select>
                    </div>
                    ...
                    <div>
                        <select
                            ...
                            onChange={(e) =>
                                setFilters({
                                    ...filters,
                                    zoneType: e.target.value as any,
                                })
                            }
                            className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-emerald-500"
                        >
                        ...
                        <tbody className="divide-y divide-slate-800/60 font-mono text-slate-200">
                            {paginatedStocks.length > 0 ? (
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3.5 text-slate-300 font-sans text-xs">
                                                {rec?.primaryZoneType ?? ""} (Rp{" "}
                                                {zonePrice.toLocaleString()})
                                            </td>

                                            {/* Entry Area */}
                                            <td className="px-4 py-3.5 text-slate-300">
                                                {entry0.toLocaleString()} -{" "}
                                                {entry1.toLocaleString()}
                                            </td>
                                ...
                                <tr>
                                    <td
                                        colSpan={9}
                                        className="p-8 text-center text-slate-400 font-sans space-y-3"
                                    >
                                        <div>
                                            No stocks match the current Smart
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
                    <div className="w-full max-w-md bg-slate-900 border border-emerald-500/40 rounded-2xl p-6 shadow-2xl shadow-emerald-500/20 text-slate-100 relative overflow-hidden text-center">
                        ...
                        <p className="text-xs text-slate-400 max-w-xs mx-auto mb-5 leading-relaxed">
                            Sedang mengambil harga & candle historis dari API
                            Pasar Saham Indonesia, serta memetakan indikator
                            FVG, Order Block & Volume Confirmation...
                        </p>
